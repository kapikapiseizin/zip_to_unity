...
fn copy_zip_to_unity(zip_file: &str, unity_folder:&str) -> (bool, String)
{
    ...
    match reset_folder(temp_folder)
    {
        Ok(()) => {},
        Err(error) => {
            return (false,  format!("{}{}{}", temp_folder, "のリセット失敗\n", error.to_string()));
        }
    }
    ...
    match extract_zip(zip_file, temp_folder)
    {
        Ok(()) => {},
        Err(error) => {
            return (false,  format!("{}{}{}", zip_file, "解凍エラー\n", error.to_string()));
        }
    }
    ...
    {
        return (false, String::from("zipからコピーしたtemp/Assetsフォルダがありません"));
    }
...
fn main() {
    ...
    .run(tauri::generate_context!()).expect("runtime error");
}
